...
import { adminAnalyticsChartsQuerySchema } from '@/validators/adminAnalyticsValidator';
...
export async function GET(req: Request) {
  try {
    ...
    const parsedQuery = adminAnalyticsChartsQuerySchema.safeParse(queryParams);
    ...
    const result = await adminAnalyticsService.getChartTrends(parsedQuery.data);
    return NextResponse.json({
      success: true,
      message: 'Chart trends loaded successfully.',
      data: result,
    });
  } catch (err: any) {
    console.error('Failed to load chart trends:', err);
    return NextResponse.json(
      { success: false, message: err.message || 'Failed to fetch chart trends.' },
      { status: 500 }
    );
  }
}
